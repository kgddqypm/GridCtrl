...
void CInPlaceList::EndEdit()
{
    ...
    if (IsWindow(pOwner->GetSafeHwnd()))
    {
        ...
        m_pCellCombo->m_nCurSel = GetCurSel();
    }
...
void CInPlaceList::OnSelChange()
{
    ...
    // 回调函数调用时可能会改变Grid, 完成当前编辑
    m_pCellCombo->EndEdit();

    if (m_pCellCombo->m_pFunComboChange != NULL)
    {
        m_pCellCombo->m_pFunComboChange(m_pCellCombo);
    }
}
...
BOOL CGridCellCombo::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
    ...
    m_pEditWnd = new CInPlaceList(GetGrid(), rect, GetStyle(), nID, nRow, nCol, 
                                  GetTextClr(), GetBackClr(), m_Strings, GetText(), nChar, this);
...
void CGridCellCombo::SetText(LPCTSTR szText)
{
    m_nCurSel = -1;
    for (int i = 0; i < m_Strings.GetSize(); ++i)
    {
        if (m_Strings[i] == szText)
        {
            m_nCurSel = i;
            break;
        }
...
void CGridCellCombo::SetCurSel(int nIndex)
{
    if (nIndex < 0 || nIndex >= m_Strings.GetSize())
        m_nCurSel = -1;
    else
    {
        m_nCurSel = nIndex;
        SetText(m_Strings[nIndex]);
    }
}
...
// ComboBox选项改变
void CGridCellCombo::SetCallBackComboChange(CallBackComboBoxChange pFun)
{
    m_pFunComboChange = pFun;
}
